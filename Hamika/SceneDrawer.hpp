...
template <typename ACTIVE_BLOCK_T>
void SceneDrawer<ACTIVE_BLOCK_T>::setRedrawnOfRow(Type::Coord::base row, Type::Coord::base begin, Type::Coord::base end)
{
	for (; begin < end; begin++)
		setRedrawnOfBlock({begin,row});
}
template <typename ACTIVE_BLOCK_T>
void SceneDrawer<ACTIVE_BLOCK_T>::setRedrawnOfCol(Type::Coord::base col, Type::Coord::base begin, Type::Coord::base end)
{
	for (; begin < end; begin++)
		setRedrawnOfBlock({col,begin});
}
...
template <typename ACTIVE_BLOCK_T>
void SceneDrawer<ACTIVE_BLOCK_T>::drawCameraMove(KIR5::BitmapTarget &target)
{
...
template <typename ACTIVE_BLOCK_T>
void SceneDrawer<ACTIVE_BLOCK_T>::drawRefreshActivity(KIR5::BitmapTarget &target)
{
...
template <typename ACTIVE_BLOCK_T>
void SceneDrawer<ACTIVE_BLOCK_T>::drawGravityLayer(KIR5::BitmapTarget &target)
{
...
template <typename ACTIVE_BLOCK_T>
void SceneDrawer<ACTIVE_BLOCK_T>::clearRedrawBlocks()
{
	map->forrange(DrawBegin, DrawEnd, [&](const Type::Coord &coord, ACTIVE_BLOCK_T &block)
	{
		if (block.Redrawn)
...
template <typename ACTIVE_BLOCK_T>
void SceneDrawer<ACTIVE_BLOCK_T>::checkRedrawRequests()
{
	map->forrange(DrawBegin, DrawEnd, [&](const Type::Coord &coord, ACTIVE_BLOCK_T &block)
	{
		if (block.object->requests.draw || (block.remain->isExists && block.remain->requests.draw))
		{
			...
			setRedrawnOfBlock(coord);
		}
	});
}

template <typename ACTIVE_BLOCK_T>
void SceneDrawer<ACTIVE_BLOCK_T>::calculatePhysicalBox(int &x1, int &x2, int &y1, int &y2, ACTIVE_BLOCK_T &block)
{
	const Type::Coord coord = block.object->GetCoord();

	const Type::Move move = block.object->GetMove();

	x1 = block.object->GetDrawCoord().x();
	y1 = block.object->GetDrawCoord().y();
	x2 = x1 + DrawSize.width();
	y2 = y1 + DrawSize.height();

	if (block.GoTo != coord)
	{
		const auto *out_object = reach(map)[block.GoTo].object;
		if (out_object->IsMoveRight())
			x2 += DrawSize.width() * out_object->GetMove().x();
		else if (out_object->IsMoveLeft())
			x1 += DrawSize.width() * out_object->GetMove().x();

		if (out_object->IsMoveDown())
			y2 += DrawSize.height() * out_object->GetMove().y();
		else if (out_object->IsMoveUp())
			y1 += DrawSize.height() * out_object->GetMove().y();
	}
}
...
void SceneDrawer<ACTIVE_BLOCK_T>::DrawBlocks(int x, int y)
{
...

	{
		...
		drawCameraMove(target);

		checkRedrawRequests();

		drawRefreshActivity(target);
		...
		map->forrange(DrawBegin, DrawEnd, [&](const Type::Coord &coord, ACTIVE_BLOCK_T &block)
		{
			if (block.Redrawn)
			{
				bool redraw_object = block.ComeFrom == coord && !block.object->events.topDraw;
				bool redraw_remain = block.remain->isExists && !block.remain->events.topDraw;

				calculatePhysicalBox(x1, x2, y1, y2, block);

				if (redraw_object || redraw_remain)
				{
					if (x2 - x1 > 0 && y2 - y1 > 0)
					{
		...
		{
			// Draws the object with topDraw flag or 
			map->forrange(DrawBegin, DrawEnd, [&](const Type::Coord &coord, ACTIVE_BLOCK_T &block)
			{
			...
			// Draws the remain with topDraw flag
			map->forrange(DrawBegin, DrawEnd, [&](const Type::Coord &coord, ACTIVE_BLOCK_T &block)
			{
				if (block.Redrawn)
				{
