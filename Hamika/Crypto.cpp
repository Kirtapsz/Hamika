...
namespace Res
{
	namespace Crypto
	{
		constexpr std::array<unsigned char, 1024> V1_1024_data = {
		...
		std::tuple<
			Encriptor<std::array<unsigned char, 1024>>
		> encriptors{
			{V1_1024, V1_1024_data}
		};


		template<typename T>
		void encrypt(KIR5::StaticStream &stream, Type type, Encriptor<T> &encriptor, bool &done)
		{
			if (!done && encriptor.type == type)
			{
				done = true;
				KIR4::encryption_pattern(&*stream.begin(), stream.size(), encriptor.pattern.data(), encriptor.pattern.size());
			}
		}
		...
		void encrypt(KIR5::StaticStream &stream, Type type)
		{
			if (type != none)
			{
				std::apply([&stream, &type](auto&&... args)
				{
					bool done = false;
					((encrypt(stream, type, args, done)), ...);
				}, encriptors);
			}
		}
	}
