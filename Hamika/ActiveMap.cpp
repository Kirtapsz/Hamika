...
namespace UI::Game
{
	...
	void ActiveMap::signVictory()
	{
		if (_account)
		{
			_account->addBluePrint(_bluePrint, loopCounter * CA * 1000);
		}
	}
	void ActiveMap::signPlay()
	{
		if (_account)
		{
	...
	void ActiveMap::finishGame()
	{
		...
		UI::MainEvent::s_object->finishGame();
	}
	void ActiveMap::playGame(const std::shared_ptr<Res::BluePrint> &bluePrint_, const std::shared_ptr<Res::Account> &account_)
	{
		...
		if (_account)
		{
			logger = std::shared_ptr<Logger>(new Logger(loopControllerInterface, _bluePrint->hash, _account->hash()));
